...
export async function GET(request: Request) {
  ...
  const bucakKoyKodu = searchParams.get("bucakKoyKodu");
  ...
  const soapEnvelope = `<?xml version="1.0" encoding="utf-8"?>
    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:das="http://daskuavt.genel.webServices.aksigorta.tr.com/">
       <soapenv:Header/>
       <soapenv:Body>
      <das:daskUavtMahalleSorgula>
         <!--Optional:-->
         <daskUavtMahalleSorgulamaInput>
            <kanalBilgileri>
               <kanalId>496</kanalId>
               <branchId>2724</branchId>
               <token>NcmwMLwBA3xlrCCr</token>
            </kanalBilgileri>
            <bucakKoyKodu>${bucakKoyKodu}</bucakKoyKodu>
         </daskUavtMahalleSorgulamaInput>
      </das:daskUavtMahalleSorgula>
       </soapenv:Body>
    </soapenv:Envelope>`;
      ...
      await parser.parseString(response.data, (err: any, result: any) => {
        ...
        res =
          result["S:Envelope"]["S:Body"][0][
            "ns2:daskUavtMahalleSorgulaResponse"
          ][0]["return"][0];
      });
    })
    .catch((error) => {
