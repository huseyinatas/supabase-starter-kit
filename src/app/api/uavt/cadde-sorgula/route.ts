...
export async function GET(request: Request) {
  ...
  const mahalleKodu = searchParams.get("mahalleKodu");
  ...
  const soapEnvelope = `<?xml version="1.0" encoding="utf-8"?>
    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:das="http://daskuavt.genel.webServices.aksigorta.tr.com/">
       <soapenv:Header/>
       <soapenv:Body>
      <das:daskUavtCaddeSokakSorgula>
         <!--Optional:-->
         <daskUavtCaddeSokakSorgulamaInput>
            <kanalBilgileri>
               <kanalId>496</kanalId>
               <branchId>2724</branchId>
               <token>NcmwMLwBA3xlrCCr</token>
            </kanalBilgileri>
            <mahalleKodu>${mahalleKodu}</mahalleKodu>
         </daskUavtCaddeSokakSorgulamaInput>
      </das:daskUavtCaddeSokakSorgula>
       </soapenv:Body>
    </soapenv:Envelope>`;
      ...
      await parser.parseString(response.data, (err: any, result: any) => {
        ...
        res =
          result["S:Envelope"]["S:Body"][0][
            "ns2:daskUavtCaddeSokakSorgulaResponse"
          ][0]["return"][0];
      });
    })
    .catch((error) => {
